...
```
Data user (org)          CPU (role: cpu)                      Datastation (role: datastation)
                    ┌──────────────────────┐                 ┌──────────────────────┐
...
A FastAPI middleware validates incoming inter-station requests on every node (both CPU and datastation):
...
For requests from data users to the CPU, the user's organizational identity must also be verified (see Data user authentication below).
The Nuts middleware activates for any cross-organizational call, whether between stations or from a data user to the CPU.
...
This has a direct implication for the CPU. If the CPU only serves users from its own organization (e.g. IKNL researchers using the IKNL CPU), one Nuts Node with one DID is sufficient. But if the CPU serves users from *multiple* organizations (an Amsterdam UMC researcher and a Leiden UMC researcher both querying the same IKNL CPU), the CPU needs a way to handle multi-org user access.

#### What this means for the data user → CPU boundary

The data user → CPU interaction is a cross-organizational boundary. A researcher from Amsterdam UMC querying the IKNL CPU must be identified as an Amsterdam UMC employee, not as a generic "IKNL user."

The Nuts-native way to handle this: the user's organization (Amsterdam UMC) has its own Nuts Node. The researcher signs a `PractitionerLogin` contract at Amsterdam UMC's Nuts Node. That VP is included in an access token request to the CPU's Nuts Node. The CPU introspects the token and gets: the user's identity (name, email) + the user's organization DID (Amsterdam UMC) + that the token was issued for `purposeOfUse: pluginlake-query-access`.

The flow looks like this:

1. Researcher opens the CPU's web UI or Python client.
2. The CPU redirects to the researcher's organization's Nuts Node (or the researcher provides their org's DID).
3. The researcher signs a `PractitionerLogin` contract (Employee Identity or IRMA).
4. Amsterdam UMC's Nuts Node obtains an access token from the CPU's Nuts Node, including the signed VP.
5. The CPU validates the token via introspection: gets org DID + user identity.
6. The CPU looks up which `NutsAuthorizationCredential`s exist for Amsterdam UMC's DID → determines which collaborations and catalogs the researcher can access.
7. Only results from those collaborations are visible to this researcher.
...
This context is used for credential-scoped access control, audit logging (who from which organization queried what), and ensuring a researcher can only see results from collaborations their organization participates in.

### Node roles: CPU and datastation
...
| Aspect | Datastation | Central Processing Unit (CPU) |
|---|---|---|
...
| Nuts auth middleware | Yes (validates CPU requests) | Yes (validates datastation responses, incoming admin calls) |
| Receives queries from | CPU (via Nuts JWT) | Data users (via org-verified auth) |
| Issues credentials to | CPU's DID (grants data access) | Nobody (receives credentials from datastations) |

A pluginlake configuration option (e.g. `role: cpu` or `role: datastation`) activates the correct modules.
The gateway enforces this role by checking `purposeOfUse` values:

- A datastation rejects outgoing requests with `purposeOfUse: pluginlake-query-dispatch` (only CPUs dispatch).
- A CPU rejects incoming requests that try to execute local query assets (it has no data).

This means there is no separate CPU codebase.
Adding a CPU to the network is deploying another pluginlake instance with a different config profile and its own Nuts Node.

### What is a collaboration?

A collaboration in the PLUGIN network is a bilateral trust agreement between a datastation (hospital) and a CPU (research coordinator).

In concrete terms, a collaboration is one digital credential: a `NutsAuthorizationCredential` issued by a datastation to a CPU.
This credential is the datastation saying: "I, Hospital X, allow CPU Y to query these specific tables, with these filters, until this date."
...
A **federation** is the collection of all bilateral credentials that point to one CPU.
When IKNL (oncology CPU) has credentials from 15 hospitals, that set of 15 credentials *is* the IKNL oncology federation.
...
- **Starting a collaboration:** the datastation operator issues a credential to the CPU's DID through the pluginlake UI.
- **Ending a collaboration:** the datastation operator revokes the credential. The CPU can no longer obtain access tokens.
...
The PLUGIN network supports many-to-many relationships between CPUs and datastations.
Multiple CPUs can exist (e.g. IKNL for oncology, DHD for quality indicators, a university for a specific study), and each CPU independently receives bilateral credentials from whichever datastations participate in its collaboration.
A single datastation can issue credentials to multiple CPUs.
...
```
    ...
    credential              credential
         ...
         └───────►│DID│◄────────┘
                  └───┘
                 CPU-B (quality registry)
```

Technically, CPU and datastation are the same pluginlake software.
What makes a node a "CPU" or a "datastation" is the combination of three things:

1. **Dagster definitions (configuration):** which asset groups and code locations are loaded. pluginlake already composes definitions via `Definitions.merge()` in `pluginlake/definitions/` (see [ADR-001](adr-001-asset-architecture.md)). A datastation loads the existing data ingestion definitions (`pluginlake.definitions.omop`, `pluginlake.definitions.fhir`) plus a new **aggregate definitions** module (`pluginlake.definitions.aggregate`) that registers query assets and the `GET /catalog` endpoint. A CPU loads a new **SDC definitions** module (`pluginlake.definitions.sdc`) that registers SDC assets (small cell suppression, k-anonymity checks, aggregation functions) and the query dispatch + `POST /queries` endpoint. These are two separate Dagster code locations that are never loaded together on the same instance.
   ...
   | `pluginlake.definitions.sdc` (new) | CPU | SDC assets (`small_cell_suppression`, `k_anonymity_check`, aggregation functions) | `POST /queries` (dispatch), `GET /catalog` (merged from datastations) |

2. **Nuts credentials:** which `NutsAuthorizationCredential` VCs the node holds and which `purposeOfUse` values those credentials grant. A datastation issues credentials to a CPU's DID, granting access to specific tables, filters, and time periods. A CPU receives these credentials and uses them to obtain OAuth2 tokens for query dispatch. The credentials define the access boundary — a CPU can only query what the credential explicitly allows.

3. **Data connections:** whether the node has a local DuckLake with patient data (datastation) or operates without local data (CPU). A datastation has `pluginlake.core.ducklake` configured with OMOP schemas and FHIR data. A CPU has no DuckLake or an empty one used only as temporary scratch space for SDC processing (no persistent patient data).
...
#### One CPU, many datastations (standard federation)

This is the primary model. One CPU (e.g. IKNL oncology) receives `NutsAuthorizationCredential`s from participating hospitals. Each credential is bilateral and scoped. The CPU aggregates results and applies SDC.

```
  Hosp. A ──credential──► CPU (IKNL oncology) ◄──credential── Hosp. B
                                ▲
                                │
                           credential
                                │
...
#### One CPU, multiple federations (multi-scope)

A single CPU DID can receive credentials with different data scopes, effectively hosting multiple logical federations on the same infrastructure. IKNL could run both an oncology federation (credentials scoped to cancer tables from 30 hospitals) and a rare disease federation (credentials scoped to different tables from 12 hospitals).

The CPU distinguishes federations by the credential's data scope. SDC is applied per federation: results from the oncology federation are never mixed with rare disease results.

This works because credentials are bilateral and self-describing. The CPU does not need separate configuration per federation — it derives the structure from the credentials it holds.

#### Multiple CPUs in the network (distinct purpose)

Multiple CPUs can coexist in the PLUGIN network, each serving a different analytical purpose. IKNL runs an oncology CPU, DHD runs a quality indicators CPU, a university runs a study-specific CPU. Hospitals issue credentials to whichever CPUs they participate with.

This is the many-to-many topology already described: each CPU is independent, each hospital decides independently which CPUs to trust.

#### Multiple CPUs for the same data (dangerous)

If two CPUs both receive credentials for the same tables from the same hospitals, both see the raw aggregate results independently. A data user with access to both CPUs could cross-reference suppressed cells between the two outputs and reconstruct data that SDC was designed to hide.

Example: CPU-A suppresses a group of 3 patients. CPU-B, running a slightly different query on the same data, returns a group that includes those 3 patients in a larger bucket. Combining both outputs reveals the hidden group.

Nuts does not prevent this — credentials are bilateral, and a hospital can issue identical scopes to multiple CPUs. This must be a governance rule: a hospital should not issue overlapping data-scope credentials to multiple CPUs unless the SDC implications are explicitly accepted.

#### No CPU (not supported)

A federation without a CPU is not possible in the PLUGIN model. The CPU has three structural responsibilities that cannot be distributed:
...
3. **Access gateway:** data users connect to the CPU, never directly to datastations. The CPU handles Nuts token acquisition on their behalf.

Without a CPU, each datastation would need to apply SDC on its own results (losing cross-hospital group size information), or data users would need direct Nuts-authenticated access to each datastation (losing the single access point and audit trail).

A peer-to-peer model without a CPU is architecturally different and not what PLUGIN proposes.

| Topology | Valid | SDC safe | Notes |
|---|---|---|---|
| 1 CPU, many datastations | Yes | Yes | Standard federation |
| 1 CPU, multiple data scopes | Yes | Yes | Separate SDC per scope |
| Multiple CPUs, distinct federations | Yes | Yes | Each CPU independent |
| Multiple CPUs, overlapping data scopes | Technically possible | No | Governance must prevent |
| No CPU | No | N/A | Contradicts PLUGIN model |
...
- The allowed `purposeOfUse` values:
  - `pluginlake-query-dispatch` (CPU→datastation): the CPU dispatches a query to a datastation for execution.
  - `pluginlake-data-serve` (datastation→CPU): the datastation returns results to the CPU.
  - `pluginlake-query-access` (consumer org→CPU): a data user's organization accesses the CPU to submit queries and view results.
...
|---|---|---|---|---|
| Datastation → CPU | Datastation | CPU's DID | `pluginlake-query-dispatch` | CPU may query specific tables, filters, time period |
| CPU → consumer org | CPU | Consumer org's DID | `pluginlake-query-access` | Org's users may submit queries and view results on this CPU |

The CPU issues `pluginlake-query-access` credentials to consumer organizations. This is the reverse direction of data access credentials: datastations grant data access *to* the CPU, and the CPU grants query access *to* consumer organizations. A consumer organization without this credential cannot obtain access tokens for the CPU.

Every station runs the same Bolt. Actual collaborations are `NutsAuthorizationCredential` VCs. Each credential encodes:

- The requesting organization's DID (the CPU).
- A data scope: which tables, column filters, and time period the CPU may query.
...
| `NutsAuthorizationCredential` | One collaboration: CPU↔datastation, with data scope | Bilateral | Private (only issuer + subject) |
...
1. A data user submits a query via the CPU (UI or Python client). The CPU verifies the user's identity and organizational DID (see Data user authentication). The CPU checks that the user's organization holds a `pluginlake-query-access` credential for this CPU.
2. The CPU validates query parameters against a Pydantic schema.
3. For each relevant datastation, the CPU's Nuts Node obtains an OAuth2 token using the `NutsAuthorizationCredential`.
4. The CPU dispatches the query asset call to each datastation with the Nuts JWT.
...
6. Results are returned to the CPU. The CPU applies SDC assets (e.g. small cell suppression for groups < k).
7. Only the SDC-processed output reaches the data user. Raw results are not stored on the CPU.

Each datastation publishes a catalog of available query assets (`GET /catalog`), so the CPU and data users know what is available per collaboration.

#### Partial failure handling

When the CPU dispatches a query to multiple datastations, some may fail or time out.
The architectural policy: **SDC correctness is only guaranteed on complete results.** If some datastations fail, the CPU returns a partial result with a manifest (which datastations contributed, which failed). The data user decides whether the partial result is usable. The CPU never silently drops failed datastations.
...
- **Async query dispatch and result polling:** Dagster runs are async. The CPU dispatches, receives a `query_id`, and polls for results. Result TTL and expiry handling are required.
- **Versioning and catalog compatibility:** the catalog includes schema versions per asset. The CPU checks compatibility before dispatching.
...
- **Rate limiting and backpressure:** datastations enforce per-CPU DID rate limits at the gateway level (not Dagster-level). `429 Too Many Requests` with backoff.
...
- Every node runs the same pluginlake software. Role (CPU or datastation) is determined by which Dagster definitions modules are loaded, which Nuts credentials the node holds, and which data connections are configured.
- Two new Dagster definitions modules will be added: `pluginlake.definitions.aggregate` (datastation: query assets, catalog) and `pluginlake.definitions.sdc` (CPU: SDC assets, query dispatch, aggregation functions).
- Each node must run a Nuts Node sidecar (~200 MB memory, negligible CPU at rest) with its own DID.
- Station operators need PKIoverheid certificates for the production network.
- A `pluginlake-federation` Bolt specification defines three `purposeOfUse` values: `pluginlake-query-dispatch` (CPU→datastation), `pluginlake-data-serve` (datastation→CPU), `pluginlake-query-access` (consumer org→CPU).
...
- Consumer organizations receive `pluginlake-query-access` credentials from the CPU to access the federation.
...
- The CPU applies SDC assets on combined results. Raw data never leaves the datastation; raw intermediate results are not stored on the CPU.
...
- All cross-organizational auth (user→CPU and CPU→datastation) is tied to Nuts organizational identity. Local admin access (CLI) may use API keys.
- Data users access the network exclusively through the CPU. The CPU verifies their organizational identity and handles Nuts token acquisition on their behalf.

## Open governance questions

The following questions require stakeholder decisions before production deployment.

### 1. Can a CPU and datastation share the same instance?
...
| SDC integrity | CPU has no local data by definition | CPU process runs alongside local patient data |
...
A collaboration starts when a datastation issues a `NutsAuthorizationCredential` to a CPU's DID.
...
The CPU applies statistical disclosure control (SDC) on combined query results before they reach the data user.
...
- Can a CPU request custom query assets, or only use what is in the catalog?
...
The CPU must verify that a data user belongs to a specific organization in the Nuts network.
...
|---|---|---|---|
| Nuts contract flow (recommended) | User signs `PractitionerLogin` at their org's Nuts Node → access token carries org DID + user identity → CPU introspects | Fully consistent with all other boundaries; no separate identity system; org identity cryptographically proven; user identity in audit trail | Every consuming org needs a Nuts Node; contract signing UX (employee identity or IRMA) must be integrated in the pluginlake UI |
| Federated OIDC + DID mapping | User logs in via institutional SSO → CPU maps OIDC issuer claim to a Nuts DID → org identity derived from mapping | Standard SSO experience; no Nuts awareness needed by the user | Requires maintaining a trusted OIDC-to-DID mapping; mapping correctness is an operational risk; two identity systems to maintain |

**Engineering recommendation:** start with the Nuts contract flow for organizations that already have a Nuts Node (which is all datastations and CPU operators).
